 ...
 *
 * Motion: the section sinks at a quarter of scroll speed while the photo pushes
 * in to 1.1×, and the display words rise out of line masks on a stagger.
 */
        ...
        <div className="relative z-10 mt-auto flex flex-col gap-[clamp(2.5rem,17vh,17rem)] pb-[clamp(2.5rem,6vh,4.5rem)] pt-[36vh]">
            ...
            <div className="border-t border-white/25 pt-[clamp(1.25rem,1.5vw,26px)]">
              <div className="grid gap-6 text-white md:grid-cols-12 md:items-start">
                <SplitLines
                  ...
                  text={hero.eyebrow}
                  delay={520}
                  className="text-fluid-body font-semibold md:col-span-3 md:max-w-[170px]"
                />
                <SplitLines
                  as="p"
                  text={hero.intro}
                  delay={600}
                  className="text-fluid-lead font-medium md:col-span-6 md:max-w-[670px]"
                />
                <a
                  href={hero.cta.href}
          ...
          <h1 className="shell display text-fluid-hero uppercase text-white">
            <span className="flex flex-wrap items-baseline justify-between gap-x-6">
              <SplitLines as="span" text={hero.words.first} delay={120} className="block" />
              <SplitLines as="span" text={hero.words.second} delay={220} className="block" />
            </span>
            <SplitLines
              as="span"
              text={hero.words.third}
              delay={320}
              className="mt-[0.06em] block pl-[max(0px,calc(27%-var(--spacing-gutter)))]"
            />
          </h1>
        </div>
      </HeroParallax>
